...
onMounted(async () => {
  const data = initData(
    'https://image.ohou.se/i/bucketplace-v2-development/uploads/cards/projects/1458712228213_iei4kBh.jpg?w=720'
  );
...
function deviceOn() {
  ...
  const layer = getLayer();
  const transformer = getTransformer();
  console.log('device on===>', getSelectedNode());
  layer.batchDraw();
}
...
function deviceOff() {
  ...
  const layer = getLayer();
  const transformer = getTransformer();
  console.log('device off===>', getSelectedNode());
  layer.batchDraw();
}
...
<template>
  <div style="margin-top: 10px">
    ...
    디바이스 :
    <el-button class="btnAdd" id="btnSMART_PLUG" @click="selectedAddDevice('SMART_PLUG')" draggable="true">
      스마트플러그
    </el-button>
  ...
  <div id="menu">
    <div>
      <button id="on-button" @click="deviceOn">On</button>
      <button id="off-button" @click="deviceOff">Off</button>
